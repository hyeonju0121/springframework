...
public class Ch09Controller {
	 ...
	 * desc, MultipartFile attach) throws IOException { // 쿼리스트링으로 넘어온 데이터는 바로 매개변수로
	 * 해석이 가능한데, // 멀티파트에서 넘어오는 데이터는 해석이 필요함. log.info("제목: " + title);
	 * log.info("설명: " + desc);
	 * 
	 * // 파일의 종류 log.info("원래 파일(사용자가 선택한 파일)이름: " + attach.getOriginalFilename());
	 * log.info("파일의 종류: " + attach.getContentType()); log.info("파일의 크기: " +
	 * attach.getSize());
	 * 
	 * // 영구적 파일로 저장 File destDir = new File("C:/Temp/uploadFiles"); if
	 * (!destDir.exists()) destDir.mkdirs(); // 폴더가 존재하지 않는 경우 디렉토리 생성
	 * 
	 * // 유일한 파일 이름 생성 (오늘의 날짜 + 파일 이름) File destFile = new File(destDir, new
	 * Date().getTime() + "-" + attach.getOriginalFilename());
	 * 
	...
	public String fileUpload(Ch09FileUploadForm form) throws IOException {
		// 쿼리스트링으로 넘어온 데이터는 바로 매개변수로 해석이 가능한데,
		// 멀티파트에서 넘어오는 데이터는 해석이 필요함.
		log.info("제목: " + form.getTitle());
		log.info("설명: " + form.getDesc());

		// 파일의 종류
		log.info("원래 파일(사용자가 선택한 파일)이름: " + form.getAttach().getOriginalFilename());
		...
		if (!destDir.exists())
			destDir.mkdirs(); // 폴더가 존재하지 않는 경우 디렉토리 생성

		// 유일한 파일 이름 생성 (오늘의 날짜 + 파일 이름)
