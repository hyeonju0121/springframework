...
public class Ch07Controller {
	...
	public ModelAndView request1(Model model) {
		...
		for (int i = 1; i <= 10; i++) {
			boardList.add(new Ch07Board(i, "제목" + i, "봄" + i, "글쓴이" + i, 0, new Date()));
		}

		ModelAndView modelAndView = new ModelAndView();
		// request 범위에 저장하기 boardList 저장
	...
	public String request2(Model model) {
		...
		// @ModelAttribute 로 jobList 객체 return (위 코드로 대체)
		/*List<String> jobList = new ArrayList<String>();
		jobList.add("개발자");
		jobList.add("공무원");
		jobList.add("은행원");
		...
		joinForm.setMid("default id");
		joinForm.setMname("홍OO");
	...
	public String sessionLogout(Model model, HttpSession session) {
		session.removeAttribute("login"); // login 세션 제거 
		return "redirect:/ch07/sessionLoginForm";
	}
